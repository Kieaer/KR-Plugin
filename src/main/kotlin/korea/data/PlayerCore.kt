...
object PlayerCore {
    fun register(name: String, uuid: String, kickCount: Int, joinCount: Int, joinDate: Long, lastDate: Long, country: String, rank: Long, permission: String, json: JsonObject, id: String, pw: String): Boolean {
        ...
        val sql = DB.database.prepareStatement(("INSERT INTO players (" + "\"name\",\"uuid\",\"admin\",\"placeCount\",\"breakCount\",\"kickCount\",\"joinCount\",\"level\",\"exp\",\"joinDate\",\"lastDate\",\"playTime\",\"attackWinner\",\"pvpWinner\",\"pvpLoser\",\"rainbowName\",\"isMute\",\"isLogged\",\"country\",\"rank\", \"permission\", \"json\", \"id\", \"pw\"" + ") VALUES (${"?,".repeat(PluginData.playerDataFieldSize)}").dropLast(1) + ")")
    ...
    fun getAllData(): Seq<PlayerData> {
        ...
        if(rs.next()) {
            result.add(PlayerData(rs.getString("name"), rs.getString("uuid"), rs.getBoolean("admin"), rs.getLong("placeCount"), rs.getLong("breakCount"), rs.getLong("kickCount"), rs.getLong("joinCount"), rs.getInt("level"), rs.getInt("exp"), rs.getLong("joinDate"), rs.getLong("lastDate"), rs.getLong("playTime"), rs.getLong("attackWinner"), rs.getLong("pvpWinner"), rs.getLong("pvpLoser"), rs.getBoolean("rainbowName"), rs.getBoolean("isMute"), rs.getBoolean("isLogged"), rs.getLong("afkTime"), rs.getString("country"), rs.getLong("rank"), JsonObject.readJSON(rs.getString("json")).asObject(), rs.getString("id"), rs.getString("pw")))
        }
    ...
    private fun getData(player: Playerc): PlayerData {
        ...
        val rs = sql.executeQuery()
        if(rs.next()) {
            data = PlayerData(rs.getString("name"), rs.getString("uuid"), rs.getBoolean("admin"), rs.getLong("placeCount"), rs.getLong("breakCount"), rs.getLong("kickCount"), rs.getLong("joinCount"), rs.getInt("level"), rs.getInt("exp"), rs.getLong("joinDate"), rs.getLong("lastDate"), rs.getLong("playTime"), rs.getLong("attackWinner"), rs.getLong("pvpWinner"), rs.getLong("pvpLoser"), rs.getBoolean("rainbowName"), rs.getBoolean("isMute"), rs.getBoolean("isLogged"), rs.getLong("afkTime"), rs.getString("country"), rs.getLong("rank"), JsonObject.readJSON(rs.getString("json")).asObject(), rs.getString("id"), rs.getString("pw"))
        }
    ...
    fun save(uuid: String): Boolean {
        ...
        val sql = DB.database.prepareStatement("UPDATE players SET " + "\"name\"=?, \"uuid\"=?, \"admin\"=?, \"placeCount\"=?, \"breakCount\"=?, \"kickCount\"=?, \"joinCount\"=?, \"level\"=?, " + "\"exp\"=?, \"lastDate\"=?, \"playTime\"=?, \"attackWinner\"=?, \"pvpWinner\"=?, \"pvpLoser\"=?, \"rainbowName\"=?, " + "\"isMute\"=?, \"isLogged\"=?, \"afkTime\"=?, \"country\"=?, \"rank\"=?, \"permission\"=?, \"json\"=? WHERE \"uuid\"=?")
